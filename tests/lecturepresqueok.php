...
/* --------------------------------------------------------------- */
/* Avant d'envoyer le gedcom, on vide les tables qui existent déjà */
/* --------------------------------------------------------------- */
...
try 
	{
	$req_suppr_db = "TRUNCATE TABLE evenements; 
				 TRUNCATE TABLE media; 
				 TRUNCATE TABLE familles; 
				 TRUNCATE TABLE individus;
				 TRUNCATE TABLE lieux;
				 TRUNCATE TABLE sources;";
	$resultat_suppr_db = $pdo->prepare ( $req_suppr_db );
	$resultat_suppr_db->execute ();
 	echo "<p>Les tables a bien été préparées.</p>";
 	}
catch ( Exception $e )
 	{
...
while ( ! feof ( $gedcom ) ) {
	...
	if (preg_match ( "/2 NAME/", $ligne ) and ($nb_individu == 0)) {
		...
		$logiciel->nom = $ligne;
		// echo "nom du logiciel: " . $logiciel->nom . "<br />";
	}
...
<h3>Auteur du Gedcom</h3>
		...
		<table>
			...
			<tr>
				...
				<td>
				<?php if (isset($logiciel->nomcomplet)) { echo $logiciel->nomcomplet; } else { echo "donnée inconnu"; } ?></td>
			</tr>
			<tr>
