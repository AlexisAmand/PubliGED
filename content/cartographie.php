...
?>
	 ...
	 * TODO : Par la suite... peut-être trouver le centre du nuage de points. 
	 */
	
	$sqlLieuOrigine = $pdo2->query ( "select * from lieux, villes_france WHERE lieux.ville LIKE villes_france.nom_commune AND lieux.ref = '1' LIMIT 1" );
	
	while ( $row = $sqlLieuOrigine->fetch ( PDO::FETCH_ASSOC ) ) 
		{
		echo "var mymap = L.map('mapid').setView([" . $row ['latitude'] . ", " . $row ['longitude'] . "], 5);";	
		}
