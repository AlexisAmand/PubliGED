...
final class FormWriter {
	...
	public function writeFormXObjects() // _putformxobjects
	{
		...
		foreach ( $this->mpdf->tpls as $tplidx => $tpl ) {
			...
			$this->writer->write ( sprintf ( '/BBox [%.2F %.2F %.2F %.2F]', $tpl ['box'] ['x'] * Mpdf::SCALE, $tpl ['box'] ['y'] * Mpdf::SCALE, ($tpl ['box'] ['x'] + $tpl ['box'] ['w']) * Mpdf::SCALE, ($tpl ['box'] ['y'] + $tpl ['box'] ['h']) * Mpdf::SCALE ) );

			if (isset ( $tpl ['box'] )) {
				$this->writer->write ( sprintf ( '/Matrix [1 0 0 1 %.5F %.5F]', - $tpl ['box'] ['x'] * Mpdf::SCALE, - $tpl ['box'] ['y'] * Mpdf::SCALE ) );
			}
			...
			if (isset ( $tpl ['resources'] )) {
				...
				if ((isset ( $this->mpdf->_res ['tpl'] [$tplidx] ['images'] ) && count ( $this->mpdf->_res ['tpl'] [$tplidx] ['images'] )) || (isset ( $this->mpdf->_res ['tpl'] [$tplidx] ['tpls'] ) && count ( $this->mpdf->_res ['tpl'] [$tplidx] ['tpls'] ))) {
